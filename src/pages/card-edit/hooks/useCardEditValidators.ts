...
const useCardEditValidators = ({ cardNumbers, expiredMonth, expiredYear, owner, cvc, pin, refs }: THookValidator) => {
  ...
  const validations: { [key: string]: [boolean, React.RefObject<HTMLInputElement | HTMLButtonElement>, string] } =
    useMemo(
      () => ({
        cardNumbers: [
          isValidCardNumber(cardNumbers.value),
          refs.cardNumber,
          '카드번호가 올바르게 입력되었는지 확인해 주세요',
        ],
        expired: [
          isValidExpiredMonth(expiredMonth.value) && isValidExpiredYear(expiredYear.value),
          refs.expired,
          '만료 연/월이 올바르게 입력되었는지 확인해 주세요',
        ],
        owner: [isValidOwner(owner.value), refs.owner, '소유자 이름이 올바르게 입력되었는지 확인해 주세요'],
        cvc: [isValidCvc(cvc.value), refs.cvc, 'CVC가 올바르게 입력되었는지 확인해 주세요'],
        pin: [isValidPin(pin.value), refs.pin, '비밀번호 앞 2자리가 올바르게 입력되었는지 확인해 주세요'],
      }),
      [...inputs, refs]
    );
